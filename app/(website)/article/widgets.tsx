...
export function Devlog({section}:{section:number}){
    ...
    if(section <= 0){
        switcherBuffer.push(
            <div className={"absolute left-2 bottom-2"}><Button text={"<-"} onClick={() => {}} disabled={true}></Button></div>
        );
    }else{
        switcherBuffer.push(
            <div className={"absolute left-2 bottom-2"}><Button text={"<-"} onClick={() => {
    ...
    if(devlogDictionary.length > (section+1)*4) {
        switcherBuffer.push(
            <div className={"absolute right-2 bottom-2"}><Button text={"->"} onClick={() => {
                const nextElement = <Devlog section={section+1}/>;
                GetRootByKey("devlogs_tab_root",document).render(nextElement);
            }} disabled={false}></Button></div>
        );
        ...
        switcherBuffer.push(
            <div className={"absolute right-2 bottom-2"}><Button text={"->"} onClick={() => {}} disabled={true}></Button></div>
        );
    }
    switcherBuffer.push(
